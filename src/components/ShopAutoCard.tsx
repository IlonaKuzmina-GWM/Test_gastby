...
const ShopAutoCard: FC<ShopAutoCardProps> = ({ title, handleClick, gatsbyImageData, slug, carInfo }) => {
    ...
    return (
        <Card className="shop-auto-card px-0 mb-3 mx-1">
            <Nav.Link href={"/" + slug} className="d-flex flex-column">
                ...
                <Card.Body className="d-flex flex-column">
                    ...
                    <Card.Footer>
                        <div className="mileage-container">{carInfo?.nobraukums && <div>{carInfo?.nobraukums} km</div>}</div>
                        <div className="price-container">€ {carInfo?.carPrice}</div>
                    </Card.Footer>
                </Card.Body>
            </Nav.Link>
        </Card>
